...
#   --include-operators   Also remove OSL/Serverless operators (cluster-scoped)
#   --delete-namespace    Delete the namespace itself at the end
#
...
delete_olm_subscriptions() {
    ...
    for sub in $(oc get subscriptions.operators.coreos.com -n "$ns" -o name 2>/dev/null); do
        echo "  Deleting $sub in $ns"
        oc delete "$sub" -n "$ns" --ignore-not-found 2>/dev/null || true
    done
}
...
post_cleanup_verify() {
    ...
    local target_ns='rhdh|orchestrator|rhdh-keycloak|knative-serving|knative-eventing|knative-serving-ingress|openshift-serverless|openshift-serverless-logic|orchestrator-infra|osl-mirror'
    local remaining_ns
    remaining_ns="$(oc get ns -o name 2>/dev/null | awk "tolower(\$0) ~ /${target_ns}/" || true)"
    ...
    echo "  Verification passed: no known orchestrator/serverless leftovers found."
}
...
if [[ "$include_operators" == "true" ]]; then
    ...
    for ns in openshift-serverless-logic openshift-serverless openshift-operators; do
        delete_olm_subscriptions "$ns"
    done

    # CSVs
    for ns in openshift-serverless-logic openshift-serverless openshift-operators; do
        for csv in $(oc get csv -n "$ns" -o name 2>/dev/null); do
