...
KEYCLOAK_URL="http://$(oc get route keycloak -n $NAMESPACE -o jsonpath='{.spec.host}')"
[ -z "$KEYCLOAK_URL" ] || [ "$KEYCLOAK_URL" = "http://" ] && echo "Error: Failed to get Keycloak route" && exit 1
